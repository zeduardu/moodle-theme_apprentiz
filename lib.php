...
function theme_apprentiz_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_apprentiz', 'preset', 0, '/', $filename))) {
    ...
    $pre = file_get_contents($CFG->dirroot . '/theme/apprentiz/scss/pre.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    $post = file_get_contents($CFG->dirroot . '/theme/apprentiz/scss/post.scss');

    // Combine them together.
    return $pre . "\n" . $scss . "\n" . $post;
}

function theme_apprentiz_update_settings_images($settingname) {
    global $CFG;
    ...
    if ($file = $fs->get_file_by_hash(sha1($fullpath))) {
        ...
        $pathname = $CFG->dataroot . '/pix_plugins/theme/apprentiz/' . $settingname . '.' . $extension;

        // This pattern matches any previous files with maybe different file extensions.
        $pathpattern = $CFG->dataroot . '/pix_plugins/theme/apprentiz/' . $settingname . '.*';

        // Make sure this dir exists.
        @mkdir($CFG->dataroot . '/pix_plugins/theme/apprentiz/', $CFG->directorypermissions, true);
